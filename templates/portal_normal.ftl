...
<head>
	...
	<link href="${themeDisplay.getPathThemeRoot()}/css/styles.css" rel="stylesheet" />
	...
	<!-- Toastr -->
    <script src="/o/my-liferay-theme/js/plugins/toastr/toastr.min.js" type="text/javascript" charset="utf-8"></script>
					...
					<#if getterUtil.getBoolean(theme_settings['display-footer']!"", false) == true>
						<div class="footer">
							<#if journalContentUtil??>
								${footer}
							<#else>							
								<#assign preferences = freeMarkerPortletPreferences.getPreferences("articleId", "${footer}") />
